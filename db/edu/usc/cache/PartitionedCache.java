...
/**
 * A partitioned cache that partitions cache size into several equal-sized
 * caches.
 * 
...
public class PartitionedCache implements Client {
	...
	public PartitionedCache(int partitions, String cacheType, long cacheSize) {
		...
		for (int i = 0; i < partitions; i++) {
			if ("slab-lru".equals(cacheType)) {
				...
				caches.add(new StatsCache(new LFUCache(cacheSize)));
			} else {
	...
	public String outputStats(long time) {
		if (time % 60 == 0) {
			...
			System.out.println(
					"cache size " + cacheSize + " process" + time + " seconds of requests took " + diff + " seconds.");
		}
