...
public class ZipfianGenerator extends NumberGenerator {
	 ...
	 * new zeta value for the larger itemcount. However, if you decrease the number
	 * of items, the code computes zeta from scratch; this is expensive for large
	 * itemsets. Usually this is not intentional; e.g. one thread thinks the number
	 * of items is 1001 and calls "nextLong()" with that item count; then another
	 * thread who thinks the number of items is 1000 calls nextLong() with
	 ...
	 * first thread incremented it. So this flag allows you to say if you really do
	 * want that recomputation. If true, then the code will recompute zeta if the
	 * itemcount goes down. If false, the code will assume itemcount only goes up,
	 * and never recompute.
	 */
	 ...
	 */
	public ZipfianGenerator(long items, Random rand) {
		this(0, items - 1, rand);
	}
	 ...
	 */
	public ZipfianGenerator(long min, long max, Random rand) {
		this(min, max, ZIPFIAN_CONSTANT, rand);
	}
	 ...
	 *            The zipfian constant to use.
	 */
	public ZipfianGenerator(long items, double zipfianconstant, Random rand) {
		this(0, items - 1, zipfianconstant, rand);
	}
	 ...
	 *            The zipfian constant to use.
	 */
	public ZipfianGenerator(long min, long max, double zipfianconstant, Random rand) {
		this(min, max, zipfianconstant, zetastatic(max - min + 1, zipfianconstant), rand);
	}
	...
	/**
	 * Create a zipfian generator for items between min and max (inclusive) for the
	 * specified zipfian constant, using the precomputed value of zeta.
	 *
	 * @param min
	 *            The smallest integer to generate in the sequence.
	 * @param max
	 ...
	 */
	public ZipfianGenerator(long min, long max, double zipfianconstant, double zetan, Random rand) {
		this.rand = rand;
		items = max - min + 1;
		base = min;
	 ...
	 * for a distribution with n items, using the zipfian constant thetaVal.
	 * Remember the value of n, so if we change the itemcount, we can recompute
	 * zeta.
	 *
	 * @param n
	...
	/**
	 * Compute the zeta constant needed for the distribution. Do this from scratch
	 * for a distribution with n items, using the zipfian constant theta. This is a
	 * static version of the function which will not remember n.
	 * 
	 * @param n
	 *            The number of items to compute zeta over.
	 * @param theta
	 ...
	 * for a distribution that has n items now but used to have st items. Use the
	 * zipfian constant thetaVal. Remember the new value of n so that if we change
	 * the itemcount, we'll know to recompute zeta.
	 *
	 * @param st
	 ...
	 * Compute the zeta constant needed for the distribution. Do this incrementally
	 * for a distribution that has n items now but used to have st items. Use the
	 * zipfian constant theta. Remember the new value of n so that if we change the
	 * itemcount, we'll know to recompute zeta.
	 * 
	 * @param st
	 *            The number of items used to compute the last initialsum
	 * @param n
	...
	public long nextLong(long itemcount) {
		...
		if (itemcount != countforzeta) {
			...
			if (itemcount > countforzeta) {
				...
				// TODO: can also have a negative incremental computation, e.g. if you decrease
				// the number of items,
				// then just subtract the zeta sequence terms for the items that went away. This
	 ...
	 * be the most popular, followed by the 1st, followed by the 2nd, etc. (Or, if
	 * min != 0, the min-th item is the most popular, the min+1th item the next most
	 * popular, etc.) If you want the popular items scattered throughout the item
	 * space, use ScrambledZipfianGenerator instead.
	 */
	@Override
